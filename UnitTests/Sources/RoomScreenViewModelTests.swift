...
class RoomScreenViewModelTests: XCTestCase {
    ...
    func testMessageGrouping() {
        ...
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[0].groupStyle, .first, "Nothing should prevent the first message from being grouped.")
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[1].groupStyle, .middle, "Nothing should prevent the middle message from being grouped.")
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[2].groupStyle, .last, "Nothing should prevent the last message from being grouped.")
    }
    ...
    func testMessageGroupingMultipleSenders() {
        ...
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[0].groupStyle, .single, "A message should not be grouped when the sender changes.")
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[1].groupStyle, .single, "A message should not be grouped when the sender changes.")
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[2].groupStyle, .first, "A group should start with a new sender if there are more messages from that sender.")
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[3].groupStyle, .last, "A group should be ended when the sender changes in the next message.")
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[4].groupStyle, .first, "A group should start with a new sender if there are more messages from that sender.")
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[5].groupStyle, .last, "A group should be ended when the sender changes in the next message.")
    }
    ...
    func testMessageGroupingWithLeadingReactions() {
        ...
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[0].groupStyle, .single, "When the first message has reactions it should not be grouped.")
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[1].groupStyle, .first, "A new group should be made when the preceding message has reactions.")
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[2].groupStyle, .last, "Nothing should prevent the last message from being grouped.")
    }
    ...
    func testMessageGroupingWithInnerReactions() {
        ...
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[0].groupStyle, .first, "Nothing should prevent the first message from being grouped.")
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[1].groupStyle, .last, "When the message has reactions, the group should end here.")
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[2].groupStyle, .single, "The last message should not be grouped when the preceding message has reactions.")
    }
    ...
    func testMessageGroupingWithTrailingReactions() {
        ...
        // Then the messages should be grouped together.
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[0].groupStyle, .first, "Nothing should prevent the first message from being grouped.")
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[1].groupStyle, .middle, "Nothing should prevent the second message from being grouped.")
        XCTAssertEqual(viewModel.state.timelineViewState.itemViewModels[2].groupStyle, .last, "Reactions on the last message should not prevent it from being grouped.")
    }

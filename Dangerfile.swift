...
let hasChangedViews = editedFiles.contains(where: { $0.lowercased().contains("/view") })
if hasChangedViews {
...
let hasPngs = editedFiles.contains(where: { $0.lowercased().contains(".xcassets") && $0.lowercased().hasSuffix(".png") })
if hasPngs {

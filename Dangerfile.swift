...
let hasChangedViews = !editedFiles.filter { $0.contains("/View") }.isEmpty
warn("Edited files \(editedFiles)")
if hasChangedViews {
    if !(danger.github.pullRequest.body?.contains("user-images") ?? true) {
//        warn("You seem to have made changes to views. Please consider adding screenshots.")
        warn("Please add screenshots or we're sending the PR into the trash 🗑")
    } else {
        warn("Apparently it has images")
    }
} else {
    warn("Hasn't changed views")
}

...
struct MapLibreMapView: UIViewRepresentable {
    ...
    /// Resolves the map's style URL for the current appearance.
    let styleURL: (_ isDarkMode: Bool) -> URL?

    let options: InteractiveMapOptions
    ...
    func updateUIView(_ mapView: MLNMapView, context: Context) {
        ...
        let dynamicMapURL = styleURL(colorScheme == .dark)
        ...
        // Compared component-wise: the app's retroactive Equatable conformance isn't visible here.
    ...
    private func setupMap(mapView: MLNMapView, with options: InteractiveMapOptions) {
    ...
    private func updateAnnotations(in mapView: MLNMapView) {
        let existingByID = Dictionary(uniqueKeysWithValues:
            (mapView.annotations ?? []).compactMap { $0 as? MapAnnotation }.map { ($0.id, $0) })
        ...
        for id in keptIDs {
            ...
            if let annotationView = mapView.view(for: existingAnnotation) as? LocationAnnotationView {
                annotationView.updateContent(with: updatedAnnotation.makeContent)
            }
        }
    }

    private func makeMapView() -> MLNMapView {
        let mapView = MLNMapView(frame: .zero, styleURL: styleURL(colorScheme == .dark))
        ...
        mapView.tintColor = options.tintColor
...
extension MapLibreMapView {
    class Coordinator: NSObject, MLNMapViewDelegate {
        ...
        private weak var followedAnnotation: MapAnnotation?
        ...
        private func startFollowingMarker(_ annotation: MapAnnotation, in mapView: MLNMapView) {

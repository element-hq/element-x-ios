...
public enum ZeroListRowPadding {
...
public struct ZeroListRow<Icon: View, DetailsIcon: View, CustomContent: View, SelectionValue: Hashable>: View {
    @Environment(\.isEnabled) private var isEnabled
    
    let label: ZeroListRowLabel<Icon>
    let details: ZeroListRowDetails<DetailsIcon>?
...
//            .listRowSeparatorTint(.compound._borderInteractiveSecondaryAlpha)
    ...
    var rowContent: some View {
        switch kind {
        case .label:
            ZeroRowContent(details: details) { label }
        ...
        case .navigationLink(let action):
            Button(action: action) {
                ZeroRowContent(details: details, accessory: .navigationLink) { label }
            }
        ...
        case .multiSelection(let isSelected, let action):
            Button(action: action) {
                ZeroRowContent(details: details, accessory: .multiSelection(isSelected)) { label }
            }
            .isToggle()
...
public struct ZeroToggleStyle: ToggleStyle {
...
public extension ZeroListRow where CustomContent == EmptyView {
    init(label: ZeroListRowLabel<Icon>,
         details: ZeroListRowDetails<DetailsIcon>? = nil,
...
public extension ZeroListRow where DetailsIcon == EmptyView, CustomContent == EmptyView {
    init(label: ZeroListRowLabel<Icon>,
         details: ZeroListRowDetails<DetailsIcon>? = nil,
         kind: Kind<CustomContent, SelectionValue>) {
        self.label = label
...
private struct ZeroRowContent<Label: View, DetailsIcon: View>: View {
    let details: ZeroListRowDetails<DetailsIcon>?
    var accessory: ZeroListRowAccessory?
    let label: () -> Label
    ...
    var body: some View {
        HStack(spacing: ZeroListRowTrailingSectionSpacing.horizontal) {
            ...
            if details != nil || accessory != nil {
                ZeroListRowTrailingSection(details, accessory: accessory)
            }
        }

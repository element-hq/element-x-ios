...
open class ThreadSummarySDKMock: MatrixRustSDK.ThreadSummary, @unchecked Sendable {
    ...
    //MARK: - latestEventDetails

    var latestEventDetailsUnderlyingCallsCount = 0
    open var latestEventDetailsCallsCount: Int {
        get {
            if Thread.isMainThread {
                return latestEventDetailsUnderlyingCallsCount
            } else {
                var returnValue: Int? = nil
                DispatchQueue.main.sync {
                    returnValue = latestEventDetailsUnderlyingCallsCount
                }
        ...
        set {
            if Thread.isMainThread {
                latestEventDetailsUnderlyingCallsCount = newValue
            } else {
                DispatchQueue.main.sync {
                    latestEventDetailsUnderlyingCallsCount = newValue
                }
    ...
    open var latestEventDetailsCalled: Bool {
        return latestEventDetailsCallsCount > 0
    }

    var latestEventDetailsUnderlyingReturnValue: ThreadSummaryLatestEventDetails!
    open var latestEventDetailsReturnValue: ThreadSummaryLatestEventDetails! {
        get {
            if Thread.isMainThread {
                return latestEventDetailsUnderlyingReturnValue
            } else {
                var returnValue: ThreadSummaryLatestEventDetails? = nil
                DispatchQueue.main.sync {
                    returnValue = latestEventDetailsUnderlyingReturnValue
                }
        ...
        set {
            if Thread.isMainThread {
                latestEventDetailsUnderlyingReturnValue = newValue
            } else {
                DispatchQueue.main.sync {
                    latestEventDetailsUnderlyingReturnValue = newValue
                }
    ...
    open var latestEventDetailsClosure: (() -> ThreadSummaryLatestEventDetails)?

    open override func latestEventDetails() -> ThreadSummaryLatestEventDetails {
        latestEventDetailsCallsCount += 1
        if let latestEventDetailsClosure = latestEventDetailsClosure {
            return latestEventDetailsClosure()
        } else {
            return latestEventDetailsReturnValue
        }
    }
}

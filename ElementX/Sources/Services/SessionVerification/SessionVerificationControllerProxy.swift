...
private class WeakSessionVerificationControllerProxy: SessionVerificationControllerDelegate {
    ...
    func didReceiveVerificationRequest(senderId: String, flowId: String) {
        proxy?.didReceiveVerificationRequest(senderID: senderId, flowID: flowId)
    }
...
class SessionVerificationControllerProxy: SessionVerificationControllerProxyProtocol {
    ...
    fileprivate func didReceiveVerificationRequest(senderID: String, flowID: String) {
        MXLog.info("Received verification request senderID: \(senderID), flowID: \(flowID)")
        
        actions.send(.receivedVerificationRequest(senderID: senderID, flowID: flowID))
    }

...
class ClassicAppMXAccount: NSObject, NSCoding {
    ...
    required init?(coder: NSCoder) {
        ...
        credentials.accessTokenExpiresAt = UInt64(coder.decodeInt64(forKey: "accessTokenExpiresAt"))
        credentials.refreshToken = coder.decodeObject(forKey: "refreshToken") as? String
        credentials.identityServer = coder.decodeObject(forKey: "identityserverurl") as? String
        credentials.identityServerAccessToken = coder.decodeObject(forKey: "identityserveraccesstoken") as? String
        credentials.deviceID = coder.decodeObject(forKey: "deviceId") as? String
        credentials.allowedCertificate = coder.decodeObject(forKey: "allowedCertificate") as? Data
        
        identityServerURL = credentials.identityServer ?? ""
        
        super.init()
        
        if let threePIDs = coder.decodeObject(forKey: "threePIDs") as? [ClassicAppMXThirdPartyIdentifier] {
            self.threePIDs = threePIDs
        }
        
        if let device = coder.decodeObject(forKey: "device") as? ClassicAppMXDevice {
            self.device = device
        }

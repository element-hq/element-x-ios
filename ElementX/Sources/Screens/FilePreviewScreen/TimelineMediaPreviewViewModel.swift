...
class TimelineMediaPreviewViewModel: TimelineMediaPreviewViewModelType {
    ...
    private func thumbnail(for item: TimelineMediaPreviewItem.Media, timeout: Duration) async -> UIImage? {
        ...
        // behind them and the viewer took 7-40s to appear. First of load / timeout / cancellation
        // resumes; a late result is dropped with the fetch.
        let arrival = Arrival()
        let image: UIImage? = await withTaskCancellationHandler {
            await withCheckedContinuation { continuation in
                let once = Once(continuation)
                arrival.once = once
                Task { await once.resume(try? load.value) }
    ...
    private final nonisolated class Arrival: @unchecked Sendable { var once: Once? }
    
    /// A continuation that resumes at most once, from whichever of load, timeout or cancellation gets there first.
    private final nonisolated class Once: @unchecked Sendable {

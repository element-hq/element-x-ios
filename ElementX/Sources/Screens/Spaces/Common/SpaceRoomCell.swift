...
struct SpaceRoomCell: View {
    ...
    enum Action { case select(String), join(String) }
    ...
    private var details: String {
        if spaceRoom.isSpace {
            L10n.screenSpaceListDetails(L10n.commonRooms(0), L10n.commonMemberCount(spaceRoom.joinedMembersCount))
        } else {
    ...
    private var accessory: some View {
        ...
        case .none, .left, .invited:
            Button(L10n.actionJoin) { action(.join(spaceRoom.id)) }
